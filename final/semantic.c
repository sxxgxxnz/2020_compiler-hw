		...
		case N_EXP_ASSIGN :
			...
			if (isAllowableAssignmentConversion(result,t,node->rlink)){
				if (isArithmeticType(result) && isArithmeticType(t))
					node->rlink=convertUsualAssignmentConversion
					(result,node->rlink);}
			else
...
brk, BOOLEAN cnt)
{
	...
	switch(node->name) {
		...
		case N_STMT_COMPOUND:
			if(node->llink)
				local_size=sem_declaration(node->llink,addr);
...
int sem_statement_list(A_NODE *node, int addr, A_TYPE *ret, BOOLEAN sw,BOOLEAN brk, BOOLEAN cnt)
{
	...
	switch(node->name) {
		case N_STMT_LIST:
			sem_statement(node->llink, addr,ret,sw,brk,cnt);
			sem_statement_list(node->rlink, addr,ret,sw,brk,cnt);
			if(size<i)
				size=i;
			break;
